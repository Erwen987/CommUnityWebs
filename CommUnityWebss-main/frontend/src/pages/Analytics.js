              ...
              {MONTHS.map((m, i) => (
                <div key={m} className="bar-group" style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'4px' }}>
                  <div style={{ display:'flex', gap:'4px', alignItems:'flex-end' }}>
                    <div className="bar requests" style={{ width:'14px', height: `${(REQUESTS[i]/MAX)*160}px`, borderRadius:'4px 4px 0 0', background:'#9dc4ff', transition:'.6s ease' }} />
                    <div className="bar reports" style={{ width:'14px', height: `${(REPORTS[i]/MAX)*160}px`, borderRadius:'4px 4px 0 0', background:'#1f5fbf', transition:'.6s ease' }} />
                  </div>
